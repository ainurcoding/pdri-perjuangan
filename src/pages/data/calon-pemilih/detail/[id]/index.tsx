// ** import react
import React from 'react'

// ** Import from next
import { useRouter } from 'next/router'
import Image from 'next/image'
import Link from 'next/link'

// ** Import MUI
import { Grid, Card, CardContent, CardHeader, TextField, Button, Box, Typography } from '@mui/material'
import { max } from 'date-fns'

const index = () => {
  // dumy data
  interface MyObject {
    property1: string
    property2: number
    property3: string
    photo: string
  }

  let myObject: MyObject = {
    property1: 'Yayan Kusnaedi',
    property2: 3190291891928198,
    property3: 'Jalan Raya Margonda No 100, Kec. Margonda, Kel. Margonda, Kota Depok, Jawa Barat',
    photo:
      'data:image/jpeg;base64,'
  }
  return (
    <Box>
      <Grid item xs={12} sx={{ marginBottom: '50px' }}>
        <Card>
          <CardContent>
            <CardHeader title='Detail Calon Pemilih'></CardHeader>
            <Grid className='d-flex flex-row justify-content-start align-items-start w-100' ml={5}>
              <Grid className='section1 col-sm-12 col-md-6 col-lg-6 col-xl-6 gap-5'>
                <Grid ml={1}>
                  <Typography variant='h6' component='h2'>
                    Nama: <Typography variant='body1'>{myObject.property1}</Typography>
                  </Typography>
                </Grid>
                <Grid ml={1}>
                  <Typography variant='h6' component='h2'>
                    NIK: <Typography variant='body1'>{myObject.property2}</Typography>
                  </Typography>
                </Grid>
                <Grid ml={1}>
                  <Typography variant='h6' component='h2'>
                    Alamat: <Typography variant='body1'>{myObject.property3}</Typography>
                  </Typography>
                </Grid>
              </Grid>
              <Grid className='section2 d-flex flex-row  col-sm-12 col-md-6 col-lg-6 col-xl-6in'>
                <Box className='d-flex flex-column justify-content-center align-items-center col-6 text-center p-auto'>
                  <Link href='/'>
                    <Image
                      src='https://image.jpnn.com/resize/1200x800-80/arsip/normal/2021/09/01/tangkapan-layar-contoh-e-ktp-yang-sudah-diberi-watermark-fot-hxj6.jpg'
                      alt='An example image'
                      height={150}
                      width={150}
                      className='mb-1 mt-1'
                    />
                  </Link>
                  <Typography className='' variant='caption'>
                    Photo KTP &#40;Click For detail&#41;
                  </Typography>
                </Box>
                <Box className='d-flex flex-column justify-content-center align-items-center col-6 text-center p-auto'>
                  <Image
                    src='data:image/jpeg;base64,'
                    alt='An example image'
                    height={150}
                    width={150}
                    className='mb-1 mt-1'
                  />
                  <Typography className='' variant='caption'>
                    Photo Profil &#40;Click For detail&#41;
                  </Typography>
                </Box>
              </Grid>
            </Grid>
          </CardContent>
        </Card>
      </Grid>
      <Grid item xs={12} sx={{ marginBottom: '50px' }}>
        <Card>
          <CardContent>
            <CardHeader title='Data Kunjungan'></CardHeader>
            <Box className='tableDataKunjungan table-responsive' ml={5}>
              <table className='table'>
                <thead>
                  <tr className='tableBlue'>
                    <th>No</th>
                    <th>Nama Caleg</th>
                    <th>Nama Relawan</th>
                    <th>Status</th>
                    <th>Catatan Kunjungan</th>
                    <th>Tanggal Kunjungan</th>
                    <th>Foto Kunjungan</th>
                  </tr>
                </thead>
                <tbody>
                  <tr>
                    <td>1</td>
                    <td>{myObject.property1}</td>
                    <td>{myObject.property1}</td>
                    <td>{myObject.property1}</td>
                    <td>{myObject.property1}</td>
                    <td>{myObject.property1}</td>
                    <td>
                      <Box className=''>
                        <Link
                          className='text-decoration-none d-flex flex-column text-center'
                          href='/data/calon-pemilih/detail/foto-kunjungan'
                          as={`/data/calon-pemilih/detail/${myObject.photo}`}
                        >
                          <Image
                            src={myObject.photo}
                            alt='An example image'
                            height={150}
                            width={150}
                            className='mb-1 mt-1'
                          />
                          <Typography className='' variant='caption'>
                            &#40;Click For detail&#41;
                          </Typography>
                        </Link>
                      </Box>
                    </td>
                  </tr>
                </tbody>
              </table>
              <Box className='text-end'>
                <Link href='/data/calon-pemilih'>
                  <Button className='bg-primary' variant='contained' size='small'>
                    Tutup
                  </Button>
                </Link>
              </Box>
            </Box>
          </CardContent>
        </Card>
      </Grid>
    </Box>
  )
}

export default index
